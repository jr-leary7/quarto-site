...
```{r}
#| code-fold: true
#| tbl-cap: Top 5 biological process GO terms per cluster
#| label: tbl-go_bp
...
We display the top 10 most-correlated genes here. For example, retinol-binding protein 4 (*Rbp4*) is a known marker of pancreatic islet cells, and of delta cells more specifically ([source](https://doi.org/10.1038/s42255-023-00876-x)).

```{r}
#| code-fold: true
#| tbl-cap: Top 10 driver genes for the peptide production gene program
...
Indeed, normalized expression of *Rbp4* is high across all mature endocrine celltypes, with delta cells showing the highest overall mean expression.

```{r}
#| code-fold: true
#| message: false
#| fig-cap: Driver gene expression by celltype
#| label: fig-driver_gene_exp
#| fig-width: 9
#| fig-height: 5
p16 <- data.frame(celltype = seu$celltype, 
                  rna = seu@assays$spliced@data["Rbp4", ]) %>% 
...
All four gene programs appear to be significantlty associated with pseudotime. 

```{r}
#| code-fold: true
#| tbl-cap: Significance testing for gene programs across the trajectory
#| label: tbl-prog_signif
