...
We split our set of `r length(dyn_genes)` trajectory DE genes into the two categories: TFs and target genes. 
...
p11 <- fortify(graph_obj_linA, 
       ...
       geom_edges(aes(linewidth = Dyn_MI, alpha = Dyn_MI, color = State), 
       ...
       scale_linewidth_continuous(range = c(0.5, 1.5), limits = c(MI_min, MI_max)) + 
       scale_alpha_continuous(range = c(0.75, 1), limits = c(MI_min, MI_max)) + 
       labs(x = "FR 1", 
            y = "FR 2", 
            color = "TF State", 
            fill = "Gene Type", 
            linewidth = "Mutual Information", 
...
p12 <- fortify(graph_obj_linB, 
               layout = layout_with_fr(graph = graph_obj_linB, 
                                       grid = "nogrid", 
                                       weights = E(graph_obj_linB)$weights^2),
               arrow.gap = .015) %>% 
       mutate(Gene_Type = if_else(name %in% hs_tfs$hgnc_symbol, "TF", "Target Gene")) %>% 
...
p13 <- fortify(graph_obj_linC, 
               layout = layout_with_fr(graph = graph_obj_linC, 
                                       grid = "nogrid", 
                                       weights = E(graph_obj_linC)$weights^2),
               arrow.gap = .015) %>% 
       mutate(Gene_Type = if_else(name %in% hs_tfs$hgnc_symbol, "TF", "Target Gene")) %>% 
...
estimateTGRN <- function(dyn.mat = NULL,
  ...
  grn_res <- foreach::foreach(g = seq(target_genes), 
                              ...
                              .options.snow = snow_opts) %dopar% {
                                ...
                                resp_var <- dyn.mat[, target_genes[g]]
                                lgbm_data <- lightgbm::lgb.Dataset(data = feature_mat, label = resp_var)
                                lgbm_cv <- lightgbm::lgb.cv(params = lgbm_params, 
                                                            data = lgbm_data, 
                                                            nrounds = 100L, 
