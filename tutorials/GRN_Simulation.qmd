...
#| fig-width: 8
#| fig-height: 6
#| fig-cap: The directed structure of the relationships between all genes - TFs, target genes, and housekeeping genes. 
#| label: fig-str3
...
```{r}
#| code-fold: true
#| tbl-cap: The accuracy of `scTIRN` when filtering to just the genes identified by `scLANE` as trajectory DE.
#| label: tbl-acc-scTIRN
...
```{r}
#| code-fold: true
#| tbl-cap: The accuracy of `epoch` when filtering to just the genes identified by `epoch` as trajectory DE.
#| label: tbl-acc-epoch
...
epoch_GRN_all <- readRDS("../../datasets/GRN_sim/epoch_GRN_all.Rds")
