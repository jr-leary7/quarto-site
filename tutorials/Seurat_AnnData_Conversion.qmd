...
ax.set_xlabel(None)
ax.set_xticklabels(violin_order, rotation=45, ha='right')
ax.set_position([0.1, 0.325, 0.8, 0.65]) 
plt.gcf().set_dpi(320)
plt.show()
```
...
We'll quickly pull the diffusion pseudotime estimate for each cell back into R using `{reticulate}`, and add it to our `Seurat` object:
