---
title: "IN PROGRESS - Interpreting Intercepts in Regression Models"
author:
  name: Jack Leary
...
This becomes easier to understand when we sum the coefficients and get the average for group **B**, which is `r library(dplyr); with_groups(cat_mod_df, X, summarise, mu = mean(y)) %>% filter(X == "B") %>% pull(mu)`. 
...
We can fit a Poisson GLM like so:
...
Thus, when we use the inverse of the link function (exponentiation) on the estimated value of $\beta_0$, we get the mean of $\mathbb{E}[\mathbf{y}] = `r mean(y)`$. 
